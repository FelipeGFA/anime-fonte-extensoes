...
for item in local_index:
    ...
    if pkg in remote_pkg_to_item:
        ...
        if item["code"] < remote_item["code"]:
            ...
            # pkg: eu.kanade.tachiyomi.animeextension.pt.example
            # module: pt.example
            module_name = pkg.replace("eu.kanade.tachiyomi.animeextension.", "")
...
def is_downgrade(filename):
    for mod in downgraded_modules:
        # Check APK pattern: aniyomi-{module}-v...
        if filename.startswith(f"aniyomi-{mod}-"):
            return True
        # Check Icon pattern: eu.kanade.tachiyomi.animeextension.{module}.png
        if filename == f"eu.kanade.tachiyomi.animeextension.{mod}.png":
            return True
    return False
...
# Save index.json (full version with hasReadme, hasChangelog and complete sources)
...
# Create index_min from index (simplified version)
index_min = []
for item in index:
    min_item = {
        "name": item["name"],
